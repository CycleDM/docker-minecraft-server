...
if [ "$add_tag" ]; then
    docker build --build-arg SERVER_TYPE=$server_type --build-arg GAME_VERSION=$game_version -t cycledm/mcserver-$server_type:$game_version -t cycledm/mcserver-$server_type:$add_tag $work_dir
else
